...
note_metric AS (
  ...
  FROM note_agg
  WHERE imp >= 5000 AND tag_imp >= 500 AND tag_click >= 30 AND buy >= 3
),
p75_thresh AS (
  SELECT note_form, percentile_approx(ctr2, 0.75) AS p75_ctr2
...
tiered AS (
  SELECT m.*,
    CASE
      ...
      WHEN m.gpm < 200 AND m.ctr2 > t.p75_ctr2 THEN '黑马'
      ELSE NULL
    END AS tier
  FROM note_metric m
...
ranked AS (
  SELECT *,
    ROW_NUMBER() OVER (PARTITION BY note_form, tier ORDER BY ctr2 DESC) AS rk
  FROM tiered
...
ORDER BY r.note_form, r.tier, r.ctr2 DESC
