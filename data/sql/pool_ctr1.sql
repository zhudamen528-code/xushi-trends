...
note_metric AS (
  ...
  FROM note_agg
  WHERE imp >= 5000 AND tag_click >= 50 AND buy >= 5
),
p75_thresh AS (
  SELECT note_form, percentile_approx(ctr1, 0.75) AS p75_ctr1
...
tiered AS (
  SELECT m.*,
    CASE
      ...
      WHEN m.gpm < 200 AND m.ctr1 > t.p75_ctr1 THEN '黑马'
      ELSE NULL
    END AS tier
  FROM note_metric m
...
ranked AS (
  SELECT *,
    ROW_NUMBER() OVER (PARTITION BY note_form, tier ORDER BY ctr1 DESC) AS rk
  FROM tiered
...
ORDER BY r.note_form, r.tier, r.ctr1 DESC
