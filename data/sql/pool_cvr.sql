...
note_metric AS (
  ...
  FROM note_agg
  WHERE imp >= 5000 AND tag_click >= 100 AND buy >= 10
),
p75_thresh AS (
  SELECT note_form, percentile_approx(cvr, 0.75) AS p75_cvr
...
tiered AS (
  SELECT m.*,
    CASE
      ...
      WHEN m.gpm < 200 AND m.cvr > t.p75_cvr THEN '黑马'
      ELSE NULL
    END AS tier
  FROM note_metric m
...
ranked AS (
  SELECT *,
    ROW_NUMBER() OVER (PARTITION BY note_form, tier ORDER BY cvr DESC) AS rk
  FROM tiered
...
ORDER BY r.note_form, r.tier, r.cvr DESC
